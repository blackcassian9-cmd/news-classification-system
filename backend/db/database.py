...
def insert_dataset(d: Dict) -> int:
    with get_conn() as conn:
        cur = conn.execute(
            """INSERT INTO datasets(name,dtype,filename,path,sample_count,num_classes,
               label_names,delimiter,encoding,in_db,status,uploaded_at)
               VALUES(?,?,?,?,?,?,?,?,?,?,?,?)""",
            (d.get("name"), d.get("dtype"), d.get("filename"), d.get("path"),
...
def get_dataset(ds_id: int) -> Optional[Dict]:
    with get_conn() as conn:
...
def count_datasets() -> Dict[str, int]:
    with get_conn() as conn:
        total = conn.execute("SELECT COUNT(*) c FROM datasets").fetchone()["c"]
        train = conn.execute("SELECT COUNT(*) c FROM datasets WHERE dtype='train'").fetchone()["c"]
        test = conn.execute("SELECT COUNT(*) c FROM datasets WHERE dtype='test'").fetchone()["c"]
        samples = conn.execute("SELECT COALESCE(SUM(sample_count),0) s FROM datasets").fetchone()["s"]
        return {"datasets": total, "train_files": train, "test_files": test, "total_samples": samples}


# -------------------- 训练运行 --------------------
def insert_run(summary: Dict, conclusions: Optional[Dict] = None) -> int:
    extra = {
    ...
    with get_conn() as conn:
        cur = conn.execute(
            """INSERT INTO runs(created_at,status,tfidf_params,nb_params,lr_params,clean_rules,
               train_count,test_count,num_classes,label_names,results,best_model,
               feature_stats,conclusions,elapsed,extra)
               VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (now_str(), "completed", _dumps(summary.get("tfidf_params")),
...
def previous_run() -> Optional[Dict]:
    """倒数第二次运行，用于'较上次'对比。"""
    with get_conn() as conn:
...
def insert_prediction(p: Dict) -> int:
    with get_conn() as conn:
        cur = conn.execute(
            """INSERT INTO predictions(created_at,text,model,pred_label,pred_name,
               confidence,topk,source) VALUES(?,?,?,?,?,?,?,?)""",
            (now_str(), p.get("text"), p.get("model"), p.get("pred_label"),
...
def count_predictions() -> int:
    with get_conn() as conn:
        return conn.execute("SELECT COUNT(*) c FROM predictions").fetchone()["c"]


def prediction_stats() -> Dict[str, int]:
    today = time.strftime("%Y-%m-%d")
    with get_conn() as conn:
        total = conn.execute("SELECT COUNT(*) c FROM predictions").fetchone()["c"]
        td = conn.execute("SELECT COUNT(*) c FROM predictions WHERE created_at LIKE ?",
                          (today + "%",)).fetchone()["c"]
        week = conn.execute(
            "SELECT COUNT(*) c FROM predictions WHERE created_at >= datetime('now','-7 day')"
        ).fetchone()["c"]
    return {"today": td, "week": week, "total": total}


# -------------------- 实验记录 --------------------
def insert_experiment(e: Dict) -> int:
    with get_conn() as conn:
        cur = conn.execute(
            """INSERT INTO experiments(name,model_type,dataset,accuracy,f1,macro_f1,
               status,created_at,description,params,run_id)
               VALUES(?,?,?,?,?,?,?,?,?,?,?)""",
            (e.get("name"), e.get("model_type"), e.get("dataset"), e.get("accuracy"),
...
def list_experiments(limit: int = 100) -> List[Dict]:
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT * FROM experiments ORDER BY id DESC LIMIT ?", (limit,)).fetchall()
        out = []
        for r in rows:
            d = dict(r)
...
# -------------------- 参数优化记录 --------------------
def insert_optimization(o: Dict) -> int:
    with get_conn() as conn:
        cur = conn.execute(
            """INSERT INTO optimizations(created_at,objective,status,best_params,
               leaderboard,history,importance,elapsed,extra)
               VALUES(?,?,?,?,?,?,?,?,?)""",
            (now_str(), o.get("objective"), o.get("status", "completed"),
...
def latest_optimization() -> Optional[Dict]:
    with get_conn() as conn:
        row = conn.execute("SELECT * FROM optimizations ORDER BY id DESC LIMIT 1").fetchone()
        if not row:
            return None
        d = dict(row)
